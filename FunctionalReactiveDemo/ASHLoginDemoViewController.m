...
- (void)viewDidLoad
{
    ...
    RACSignal *enabledSignal = [RACSignal combineLatest:@[self.emailTextField.rac_textSignal, self.passwordTextField.rac_textSignal]
                                                 reduce:^id(NSString *emailAddress, NSString *password){
                                                     BOOL emailValid = [emailAddress rangeOfString:@"@"].location != NSNotFound;
                                                     BOOL passwordValid = password.length > 6;
                                                     return @(emailValid && passwordValid);
                                                 }];
    
    self.button.rac_command = [[RACCommand alloc] initWithEnabled:enabledSignal
